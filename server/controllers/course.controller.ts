...
export const uploadCourse = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.log("before cloud of creating course")
      if (thumbnail) {
        const myCloud = await cloudinary.v2.uploader.upload(thumbnail, {
          folder: "courses",
        });
        console.log("after cloud of creating course")
        data.thumbnail = {
...
//get single course - without purshasing
...
//get course content -- only foe valid user
...
if(!courseExists){
      return next(new ErrorHandler("you are not eligibal for this course", 404));
}
...
export const addReview= CatchAsyncError(async(req:Request, res:Response, next:NextFunction)=>{
   ...
   //check if courseId already exist in userCourseList based on_id
   ...
   if(course){
    course.ratings= avg / course.reviews.length; //suppose we have 2 reviews one is 5 another is 4 so 9/2=4.5 ratings
   }
await course?.save();
const notification={
...
//get all the courses -- only for admin
